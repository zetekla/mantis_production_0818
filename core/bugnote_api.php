...
function show_note($p_bugnote_id, $p_inline = null){
	$response = bugnote_get_text($p_bugnote_id);
	if (!empty($p_inline)){
		preg_match_all('/src=[("|\')]([^"&&^\']+)[("|\')]/', $response, $src_array);
		...
		foreach ($t_cid_array as $cid){
			...
			foreach ($p_inline as $value) {
				...
				if (preg_match("/$pre_str/", $cid, $match)){
					...
					$response = str_replace($cid, $t_img_url, $response);
				}
			}
		}
		$result = $response . '<br/> Email HTML Generated';
	} 
	// added to resolve no result value when inline was null or empty.
	else {
		$result = $response . '<br/> Email HTML Generated Without Images';
	}
	return $result ;
}
